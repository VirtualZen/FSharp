...
let rec private insert target newValue =
    ...
    then { target with left = visit target.left newValue }
    else { target with right = visit target.right newValue }
...
let sortedData (root: node) = getList (Some root)
